...
export default function EmailBlock() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Connect to email API / form service (e.g. Resend, Formspree)
    setSubmitted(true);
  ...
  return (
    <div className="flex flex-col gap-4 rounded-sm bg-ocean-alt p-5 text-white">
      <div className="flex gap-4">
        {/* Left: label */}
        <p className="w-24 shrink-0 font-serif text-lg leading-snug">
          {copy.heading}
        </p>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-1 flex-col gap-2">
            ...
            <div className="relative">
              ...
              <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-[#b3b3b3]">
                ▼
              </span>
            </div>
            <textarea
            ...
            <div className="flex justify-end">
              <Button className="px-6 py-1.5 text-sm">{copy.submit}</Button>
            </div>
          </form>
        )}
      </div>
