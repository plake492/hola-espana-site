...
export default function Hero() {
  return (
    <section className="relative flex min-h-[max(calc(100dvh+250px),950px)] w-full flex-col items-center justify-center bg-black text-white" id="hero">
      ...
      <div className="relative z-1 mx-2 flex h-full flex-col gap-16 pb-24">
        <h1 className="flex flex-col items-start gap-8 tracking-widest whitespace-nowrap">
          <span className="text-headline2">{heroCopy.header.first}</span>
          <span className="text-headline1 ml-(--headline-ml-offset) uppercase max-[950px]:ml-[-2.75px]">{heroCopy.header.second}</span>
          <span className="text-headline2 ml-(--headline-ml-offset)">{heroCopy.header.third}</span>
        </h1>
        <p className="text-headline-sub ml-(--headline-ml-offset) text-2xl italic">{heroCopy.subHeader}</p>
      </div>
      <Tear />
    </section>
  );
